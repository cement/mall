...
public class GenreContentFragment extends Fragment {
	...
	private GenreContentFragment(List<DataModel> datas) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		genreContent.setLayoutManager(new GridLayoutManager(getContext(),3));
		genreContent.addItemDecoration(new ItemDivider(getContext(), ItemDivider.VERTICAL_LIST));
		genreContent.setAdapter(new ContentAdapter());
		return genreContent;
	}
	
	class ContentAdapter extends RecyclerView.Adapter<BaseViewHolder>{
	...
	class ContentViewHolder extends BaseViewHolder{
		...
		public void bind(int position) {
			// TODO Auto-generated method stub
			textView.setText(mDatas.get(position).name+"�� "+position);
			descView.setText(mDatas.get(position).desc+"�� "+position);
